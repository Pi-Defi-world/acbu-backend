...
export async function getGovernmentTreasury(
  ...
  try {
    ...
    const totalAcbu = (minted._sum.acbuAmount?.toNumber() ?? 0) - (burned._sum.acbuAmountBurned?.toNumber() ?? 0);
    res.status(200).json({
      totalBalanceAcbu: totalAcbu,
      byCurrency: [],
      message: 'Government treasury view. Investment allocation and yield will appear when implemented.',
    });
  } catch (e) {
    next(e);
  }
